...
class Ball:
    ...
    def move(self):
        ...
        if self.x > 800 - self.r or self.x < self.r:
            self.vx = -self.vx * const_tr
        if self.y > 600 - self.r:
            self.y = 600 - self.r
            self.vy = -self.vy * const_tr
        # тут конечно можно получше описать трение, но .....
        # и там есть баг с тем что шарик застревает в правой стенке...
        self.set_coords()
    ...
    def hittest(self, ob):
        ...
        """
        # TODO
        return math.sqrt((self.x - ob.x) ** 2 + (self.y - ob.y) ** 2) <= (self.r + ob.r)
...
class gun:
    ...
    def __init__(self, x=40, y=450):
        ...
        self.f2_on = 0
        self.an = 1
        self.id = canv.create_line(self.x, self.y, self.x + 30 * math.cos(self.an), self.y + 30 * math.sin(self.an), width=7)
        # тут я не понял что надо довести и просто сделал через координаты -_-
...
class target:
    ...
    def new_target(self):
        ...
        r = self.r = rnd(20, 50)
        # я по маленькой не попадаю => 2 -> 20
